...
void GraficaV(void){
  cout<<"set term pdf"<<endl; 
  cout<<"set out 'HistoVel.pdf'"<<endl;
  ...
  cout<<"set ylabel 'Numero de particulas'"<<endl;
	...
	cout<<"set font ',7'"<<endl;
	cout<<"gauss(x)=a/(sigma*sqrt(2.*pi))*exp(-(x-mu)**2./(2.*sigma**2))"<<endl;
	cout<<"fit gauss(x) 'HistoV.txt' via a, sigma, mu"<<endl;
	cout<<"plot 'HistoV.txt' u 1:2   with impulses t 'Datos', gauss(x)"<<endl;
}
...
int main(){
  ...
  Vx<<"sigma_v="<<sigma_v<<endl;
	...
	double sigma = 0.5;
	auto min=setV.begin(), max = setV.end();
